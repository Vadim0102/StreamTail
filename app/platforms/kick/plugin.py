"""
Kick.com Integration Plugin.

========================================================================================
                                     KICK API MAP
========================================================================================

1. ЧТЕНИЕ СТАТУСА (Официальный API - Без Cloudflare защиты):
   • GET https://api.kick.com/public/v1/channels?slug={slug}
   • Headers: Authorization: Bearer <access_token>
   • Response: {"data": [{"stream_title": "...", "category": {"name": "..."}, "stream": {"is_live": true, "viewer_count": 15}}]}

2. ЧТЕНИЕ СТАТУСА (Резервный веб-интерфейс - Подвержен Cloudflare проверкам TLS-отпечатков):
   • GET https://kick.com/api/v1/channels/{slug}
   • Headers: Cookie: ...; Referer: https://kick.com/
   • Response: {"livestream": {"viewer_count": 15, "session_title": "...", "categories": [{"name": "..."}]}}

3. ПОИСК КАТЕГОРИЙ (Официальный API):
   • GET https://api.kick.com/public/v2/categories?name={game_name}
   • Response: {"categories": [{"id": 101, "name": "Just Chatting"}]}

4. ПОИСК КАТЕГОРИЙ (Резервный веб-интерфейс):
   • GET https://kick.com/api/v1/categories?name={game_name}

5. ОБНОВЛЕНИЕ СТРИМА (Официальный API - Метод PATCH, требует OAuth-токен):
   • PATCH https://api.kick.com/public/v1/channels
   • Headers: Authorization: Bearer <token_с_правами_channel:write>; Content-Type: application/json
   • Body (JSON): {"stream_title": "New Title", "category_id": int(game_id)}

6. ОБНОВЛЕНИЕ СТРИМА (Неофициальный метод - Требует куки браузера):
   • PUT https://kick.com/api/v2/channels/{slug}
   • Headers: Cookie: ...; X-XSRF-TOKEN: ...; Content-Type: application/json
     * Важно: Значение 'X-XSRF-TOKEN' автоматически парсится плагином из скопированных кук!
   • Body (JSON): {"stream_title": "New Title"} или {"category_id": int(game_id)}
========================================================================================
"""
